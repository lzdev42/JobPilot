...
fun MainPage(modifier: Modifier) {
    val tabItems = listOf<String>("boss","51job","猎聘","个人简介","Prompts","求职红线","求职偏好","设置")
    ...
    val rejectionRulesViewModel = remember { InputViewModel(InputType.REJECTION_RULES) }
    val preferenceRulesViewModel = remember { InputViewModel(InputType.PREFERENCE_RULES) }
    ...
    Column(modifier = modifier) {
        ...
        Box(modifier = Modifier.weight(1f)) {
            when (selectedTabIndex) {
                ...
                5 -> InputView(rejectionRulesViewModel,Modifier.fillMaxSize())
                6 -> InputView(preferenceRulesViewModel,Modifier.fillMaxSize())
                7 -> SettingView(settingViewModel,Modifier.fillMaxSize())
            }
        }
    }
}